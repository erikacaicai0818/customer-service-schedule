...
st.set_page_config(page_title="客服考勤管理系统 v19.0", layout="wide", page_icon="⚖️")
...
st.title("🛡️ 客服部智能化管理系统 v19.0 (基准模板版)")
...
with tabs[1]:
    ...
    t_staff = i_c1.selectbox("客服人员", STAFF, key="is_p")
        ...
        for i, d in enumerate(week):
            if d.month == t_month:
                ...
                with cols[i]:
                    ...
                    st.markdown(f"<div class='calendar-cell {is_today}'><div style='font-size:1.1em; font-weight:bold;'>{d.day}</div><div style='color:#004085; margin-top:5px; font-weight:bold;'>{'休' if f_dt=='休息' else disp}</div></div>", unsafe_allow_html=True)
                    if f_dt != "休息":
                        l_t = "12:00" if f_dt == "早班" else "13:00" if f_dt == "延迟班" else "无午休"
                        st.caption(f"🍱{l_t}")
                        if not db_v.empty:
...
with tabs[2]:
    ...
    for name in STAFF:
        ...
        for d in w_dates:
            ...
            # 文本构建
            cell_txt = f"🔄{after_swap}\n(原:{orig})" if after_swap != orig else f"{after_swap}"
            ...
            if not leave_recs.empty:
                ...
                if l_h >= FIXED_HOUR: cell_txt = f"🚫{l_type}\n(原:{orig})"
                else: cell_txt += f"\n(-{l_h}h {l_type})"
            else: real_h += day_theo
...
with tabs[3]:
    ...
    for name in STAFF:
        ...
        m_summary.append({"姓名": name, "出勤天数": act_wd, "理论工时": f"{std}h", "事假扣除": f"-{pers}h", "应发工时": f"{round(max(0.0, std - pers),1)}h"})
    st.table(pd.DataFrame(m_summary))

# Tab 5: 管理记录
with tabs[4]:
    st.subheader("🔍 流水记录回撤中心")
