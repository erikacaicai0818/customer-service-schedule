...
st.set_page_config(page_title="客服考勤综合管理系统 v22.3", layout="wide", page_icon="⚖️")
...
def save_data_to_gsheets(new_row_dict):
    ...
    # 【核心修复】：在这里自动计算唯一 ID，防止重复
    if not df.empty:
        # 强制将 ID 转为数字计算最大值
        max_id = pd.to_numeric(df['id'], errors='coerce').max()
        new_row_dict['id'] = int(max_id + 1) if not pd.isna(max_id) else 1001
    else:
        new_row_dict['id'] = 1001
...
with st.sidebar:
    ...
    with st.form("holiday_form", clear_on_submit=True):
        ...
        if st.form_submit_button("确认节日值班"):
    ...
    for week in weeks:
        ...
        for i, d in enumerate(week):
            if d.month == t_m:
                ...
                h_n = HOLIDAYS_2026.get(d)
                is_t, is_h = ("calendar-today" if d==now_beijing.date() else ""), ("calendar-holiday" if h_n else "")
                ...
                with cols[i]:
                    ...
                    st.markdown(f"<div class='calendar-cell {is_t} {is_h}'><div style='font-size:1.1em; font-weight:bold;'>{d.day} {star}{phone}</div><div style='color:red; font-size:0.7em;'>{h_n if h_n else ''}</div><div style='color:#004085; font-weight:bold;'>{'休' if f_dt=='休息' else disp}</div></div>", unsafe_allow_html=True)
                    if f_dt != "休息":
...
with tabs[2]:
    ...
    s_m = cw1.selectbox("月份筛选", range(1, 13), index=now_beijing.month-1, key="wm_223")
...
with tabs[3]:
    st.subheader("💰 月度计薪汇总")
    m_y, m_m = st.selectbox("年份汇总", [2026, 2027], key="sy223"), st.selectbox("月份汇总", range(1, 13), index=now_beijing.month-1, key="sm223")
...
# Tab 5: 管理记录 (多重 Key 保护)
with tabs[4]:
    st.subheader("🔍 流水记录查询与回撤")
    f_m = st.selectbox("流水月份", range(1, 13), index=now_beijing.month-1, key="fm_223")
    if not db_full.empty:
        ...
        # 使用 enumerate 增加一个额外的 index 保证按钮 Key 绝对唯一
        for idx, (row_idx, r) in enumerate(df_s.iterrows()):
            ...
            # 【修复关键点】：key 使用了 id + 循环索引的双重保险
            unique_key = f"rev_{r['id']}_{idx}"
            if str(r['status']) == "有效" and c3.button("撤回", key=unique_key):
                withdraw_req(r['id'])
                st.rerun()
            st.divider()
